...
namespace Duftfinder.Web.Models
{
	...
	public class EssentialOilViewModel
	{
		...
		/// </summary>
		/// <author>Anna Krebs</author>
		public string FirstLetterOfEssentialOilName => Name.Substring(0, 1).ToUpper();

		public string NameLatin
		{
			get => _essentialOil.NameLatin;
			set => _essentialOil.NameLatin = value;
		}

		public string Description
		{
			get => _essentialOil.Description;
		...
		public string Type
		{
			get
			{
				...
				if (string.IsNullOrEmpty(_essentialOil.Type)) return EssentialOilType.Oil.ToString();
		...
		/// </summary>
		/// <author>Anna Krebs</author>
		public string EssentialOilTypeDisplayName => _essentialOilType?.Display();
		...
		private void SetPictureValues(EssentialOil essentialOil)
		{
			...
			var isSamePicture = PictureDataAsString != null && essentialOil.PictureDataAsString == null;
			var isSamePictureFileName = PictureFileName != null && essentialOil.PictureFileName == null;

			// Keep already uploaded picture if edit & picture wasn't removed.
			if (isSamePicture) essentialOil.PictureDataAsString = PictureDataAsString;

			// Keep already uploaded picture file name if edit & picture wasn't removed.
			if (isSamePictureFileName) essentialOil.PictureFileName = PictureFileName;

			// Remove PictureSource text if no picture is set.
			var noPictureSet = PictureDataAsString == null && essentialOil.PictureDataAsString == null;
			if (noPictureSet) essentialOil.PictureSource = null;
		}
	}
}