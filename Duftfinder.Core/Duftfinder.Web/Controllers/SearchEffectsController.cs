...
namespace Duftfinder.Web.Controllers
{
    ...
    public class SearchEffectsController : Controller
    {
        ...
        {
            SearchEffectsViewModel searchEffectsViewModel = new SearchEffectsViewModel();          
        ...
        public async Task<ActionResult> SearchEssentialOil(List<SearchEffectItem> searchEffects)
        {
            ...
            int maxEffectDegreeDiscomfortValue = 0;
            foreach (SearchEffectItem searchEffect in searchEffects)
            {
            ...
            IList<SearchEssentialOilItem> searchEssentialOilItemsResults = await _essentialOilService.GetEssentialOilResultsBySearchedEffectsNameAsync(searchEffects);
            ...
            IList<EssentialOilViewModel> essentialOilViewModels = CreateEssentialOilViewModels(searchEssentialOilItemsResults, maxEffectDegreeDiscomfortValue);
        ...
        /// <returns></returns>
        private IList<EssentialOilViewModel> CreateEssentialOilViewModels(IList<SearchEssentialOilItem> searchEssentialOilItemsResults, int maxEffectDegreeDiscomfortValue)
        {
            ...
            foreach (SearchEssentialOilItem resultItem in searchEssentialOilItemsResults)
            {
                // Get the specific result item and add to view model.
                EssentialOilViewModel essentialOilViewModel = new EssentialOilViewModel(resultItem.EssentialOil);
                ...
                essentialOilViewModel.WeightedMatchValue = _essentialOilService.CalculateWeightedMatchValue(resultItem, maxEffectDegreeDiscomfortValue);
        ...
        private List<SearchEffectItem> RemoveDuplicateSearchEffects(List<SearchEffectItem> searchEffects)
        {
            ...
            List<IGrouping<string, SearchEffectItem>> duplicateSearchedEffects = searchEffects.GroupBy(s => s.SearchEffectText).Where(g => g.Count() > 1).ToList();

            // Remove all duplicate searched effects except the one with the highest DiscomfortValue.
            foreach (IGrouping<string, SearchEffectItem> duplicateSearchedEffect in duplicateSearchedEffects)
            {
                SearchEffectItem duplicateWithHighestDiscomfortValue = searchEffects.Where(se => se.SearchEffectText == duplicateSearchedEffect.Key).OrderByDescending(s => s.DiscomfortValue).FirstOrDefault();
