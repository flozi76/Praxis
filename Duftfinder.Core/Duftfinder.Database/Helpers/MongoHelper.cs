...
namespace Duftfinder.Database.Helpers
{
	...
	public static class MongoHelper
	{
		...
		private static readonly FindOptions FindOptions = new FindOptions
	...
	public static class MongoHelper<TEntity> where TEntity : Entity
	{
		...
		/// <returns></returns>
		public static async Task<bool> IsDuplicate(string primaryName, string primaryValue,
			IMongoCollection<TEntity> collection, ObjectId? objectId)
		{
			// Don't check for duplicates, if no primaryName is set.
			if (string.IsNullOrEmpty(primaryName) || string.IsNullOrEmpty(primaryValue)) return false;

			// Filter to check duplicate of entity. primaryValue has to be exact match in collection. 
			var filter = Builders<TEntity>.Filter.Eq(primaryName, primaryValue.Trim());
			var documentList = await collection.Find(filter, FindOptions).ToListAsync();

			// Entity doesn't exist is database.
			if (documentList.Count == 0) return false;

			// The entity itself was updated and therefore already exists in database.
			if (documentList.Count == 1)
			{
				var entityList = DeserializeBsonDocumentsToEntities(documentList);
				var isSameUpdateEntity = entityList.SingleOrDefault()?.ObjectId == objectId;

				if (isSameUpdateEntity) return false;
			}
