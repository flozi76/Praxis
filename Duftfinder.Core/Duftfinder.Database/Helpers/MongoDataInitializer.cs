...
namespace Duftfinder.Database.Helpers
{
	...
	public class MongoDataInitializer
	{
		...
		public async Task InitEssentialOils()
		{
			...
			await InitEssentialOilCollection("Indianernessel", "Monarda fistulosa L. Ct. geraniol",
				EssentialOilType.Oil);
			await InitEssentialOilCollection("Ingwer", "Zingiber officinalis  ", EssentialOilType.Oil);
			await InitEssentialOilCollection("Jasmin, 10% in Jojobaöl", "Jasminum grandiflorum  ",
				EssentialOilType.Oil);
			...
			await InitEssentialOilCollection("Petitgrain Mandarine", "Citrus reticulata (Blätter)",
				EssentialOilType.Oil);
			...
			await InitEssentialOilCollection("Rosmarin Ct. cineol", "Rosmarinus officinalis Ct. cineol",
				EssentialOilType.Oil);
			await InitEssentialOilCollection("Rosmarin Ct. verbenon", "Rosmarinus officinalis Ct. verbenon",
				EssentialOilType.Oil);
			await InitEssentialOilCollection("Sandelholz", "Santalum austrocaledonicum", EssentialOilType.Oil);
			await InitEssentialOilCollection("Thymian Ct. linalool", "Thymus vulgaris Ct. linalool",
				EssentialOilType.Oil);
		...
		public async Task InitEffects()
		{
			...
			await InitEffectCollection("GABA: Rezeptorenempfindlichkeit steigernd",
				CategoryValue.NervousSystemNeurotransmitter);
			await InitEffectCollection("Noradrenalin: Ausschüttung anregend",
				CategoryValue.NervousSystemNeurotransmitter);
			await InitEffectCollection("Serotonin (aktivierend Raphus nucleus)",
				CategoryValue.NervousSystemNeurotransmitter);
			...
			await InitEffectCollection("Enterisches Nervensystem balancierend (Stress und Ängste)",
				CategoryValue.EntericNervousSystem);
			...
			await InitEffectCollection("tröstend, Geborgenheit vermittelnd, schützend",
				CategoryValue.IntegrationYoungerSelf);
			...
			await InitEffectCollection("Besänftigend, aufmunternd, Zuversicht entwickeln",
				CategoryValue.BlockageDissolving);
		...
		public async Task InitUsersAndRoles()
		{
			...
			// Initialize Users
			// Hash PW for: 123456
			await InitUserCollection("duftfinder@gmail.com",
				"20000:QMzEVU8vsURxmfyrFYOuSg==:zGYaINivZ+y98K7y6vPJUextJCE=", true, true, false, RoleValue.Admin,
				"Barbara", "Held", true);
		}
		...
		public async Task InitConfigurationValues()
		{
			...
			await InitConfigurationCollection("smtpEmailUserPassword", "MtDuftfinder?2018",
				"Passwort des E-Mail-Accounts des Absenders (in Klartext).");
			await InitConfigurationCollection("smtpEmailFrom", "duftfinder@gmail.com", "Absendername");
			await InitConfigurationCollection("verifyAccountEmailSubject", "Ihre Registrierung bei Duftfinder",
				"\"Betreff-Text\" vom E-Mail, welches der User nach seiner Registration erhält.");
			await InitConfigurationCollection("verifyAccountEmailText",
				"Guten Tag<p>Vielen Dank f&uuml;r Ihre Registration bei Duftfinder. Um die Registration zu best&auml;tigen, klicken Sie bitte den folgenden Link: <a href={0} target=\"_blank\">{0}</a></p><p>Ihre Registration muss daraufhin von der Administratorin best&auml;tigt werden. Dies kann einige Tage dauern. Sobald Ihr Account freigeschalten wurde, werden Sie per E-Mail informiert. Ihr Account wird erst dann nutzbar sein.</p><p>Mit freundlichen Gr&uuml;ssen und bis bald auf Duftfinder</p><p>Barbara Held</p>",
				"E-Mail-Text vom E-Mail, welches der User nach seiner Registration erhält. {0} ist der Platzhalter für den Verifizierungs-Link, den der neue User in der E-Mail erhält und darf nicht entfernt werden.");
			await InitConfigurationCollection("infoAboutRegistrationEmailSubject", "Neuer Benutzer registriert",
				"\"Betreff-Text\" vom E-Mail, welches der Admin nach der Registrationsbestätigung eines neuen Users erhält.");
			await InitConfigurationCollection("infoAboutRegistrationEmailText",
				"Hallo Barbara<p>Soeben hat ein Benutzer mit der E-Mail-Adresse {0} seine Registration in Duftfinder verifiziert.</p><p>Geh doch bei Gelegenheit einmal in die Benutzerverwaltung von Duftfinder und pr&uuml;fe, ob du diesen Benutzer best&auml;tigen m&ouml;chtest.</p><p>Liebe Gr&uuml;sse von der Entwicklerin von Duftfinder</p>",
				"E-Mail-Text vom E-Mail, welches der Admin nach der Registrationsbestätigung eines neuen Users erhält. {0} ist der Platzhalter für die E-Mail-Adresse des neuen Users und darf nicht entfernt werden.");
			await InitConfigurationCollection("infoAboutRegistrationConfirmationSubject", "Registration abgeschlossen",
				"\"Betreff-Text\" vom E-Mail, welches der User nach der Registrationsbestätigung vom Admin erhält.");
			await InitConfigurationCollection("infoAboutRegistrationConfirmationText",
				"Guten Tag<p>Ihre Registration bei Duftfinder wurde soeben von der Administratorin best&auml;tigt. Sie k&ouml;nnen sich nun unter <a href={0} target=\"_blank\">{0}</a> einloggen.</p><p>Mit freundlichen Gr&uuml;ssen und bis bald auf Duftfinder</p><p>Barbara Held</p>",
				"E-Mail-Text vom E-Mail, welches der User nach seiner Registration erhält. {0} ist der Platzhalter für den Link, um das Passwort zu ändern und darf nicht entfernt werden.");
			await InitConfigurationCollection("forgotPasswordEmailSubject", "Passwort Reset",
				"\"Betreff-Text\" vom E-Mail, welches der User erhält, wenn er sein Passwort vergessen hat und zurücksetzen will.");
			await InitConfigurationCollection("forgotPasswordEmailText",
				"Guten Tag<p>Sie haben das &Auml;ndern Ihres Passworts bei Duftfinder beantragt. Um Ihr Passwort zu &auml;ndern, klicken Sie bitte den folgenden Link: <a href={0} target=\"_blank\">{0}</a></p><p>Ihr Passwort kann daraufhin neu gesetzt werden.</p><p>Mit freundlichen Gr&uuml;ssen und bis bald auf Duftfinder</p><p>Barbara Held</p>",
				"E-Mail-Text vom E-Mail, welches der User nach erhält, wenn er sein Passwort vergessen hat und zurücksetzen will. {0} ist der Platzhalter für den Verifizierungs-Link, den der User in der E-Mail erhält und darf nicht entfernt werden.");
		}
		...
		{
			var substanceValueString = substanceValue.ToString();
			var sortOrder = Convert.ToInt32(substanceValue);

			var substanceCollection = _mongoContext.Database.GetCollection<Substance>(nameof(Substance));

			var isDuplicate =
				await MongoHelper<Substance>.IsDuplicate(Constants.Name, substanceValueString, substanceCollection,
					null);

			if (!isDuplicate)
			{
				var substance = new Substance {Name = substanceValueString, SortOrder = sortOrder};
		...
		private async Task InitIsGeneralMoleculeCollection(SubstanceValue substanceValue, ObjectId substanceId)
		{
			...
			var moleculeCollection = _mongoContext.Database.GetCollection<Molecule>(nameof(Molecule));
			var isDuplicate =
				await MongoHelper<Molecule>.IsDuplicate(Constants.Name, substanceValueString, moleculeCollection, null);

			if (!isDuplicate)
			{
				// Inserts a general molecule for the substance.
				await moleculeCollection.InsertOneAsync(new Molecule
					{Name = substanceValueString, IsGeneral = true, SubstanceId = substanceId});
		...
		private async Task InitEssentialOilCollection(string name, string nameLatin, EssentialOilType essentialOilType)
		{
			...
			if (!isDuplicate)
			{
				var essentialOil = new EssentialOil {Name = name, NameLatin = nameLatin, Type = essentialOilTypeString};
		...
		private async Task InitEffectCollection(string name, CategoryValue categoryValue)
		{
			...
			if (!isDuplicate)
			{
				...
				var effect = new Effect {Name = name, CategoryId = category.ObjectId};
		...
		private async Task InitMoleculeCollection(string name, SubstanceValue substanceValue)
		{
			...
			var moleculeCollection = _mongoContext.Database.GetCollection<Molecule>(nameof(Molecule));
			var isDuplicate = await MongoHelper<Molecule>.IsDuplicate(Constants.Name, name, moleculeCollection, null);

			if (!isDuplicate)
			{
				// Get substance by name.
				var substanceCollection = _mongoContext.Database.GetCollection<Substance>(nameof(Substance));
				var bsonFilter = Builders<Substance>.Filter.Eq(nameof(Substance.Name), substanceValueString);
				var substance = await substanceCollection.Find(bsonFilter, _findOptions).SingleOrDefaultAsync();

				var molecule = new Molecule {Name = name, SubstanceId = substance.ObjectId};
		...
		private async Task InitRoleCollection(RoleValue roleValue)
		{
			...
			if (!isDuplicate)
			{
				var role = new Role {Name = roleValueString};
		...
		{
			var configurationCollection = _mongoContext.Database.GetCollection<Configuration>(nameof(Configuration));

			var isDuplicate =
				await MongoHelper<Configuration>.IsDuplicate(Constants.Key, key, configurationCollection, null);

			if (!isDuplicate)
			{
				var configuration = new Configuration {Key = key, Value = value, Description = description};
