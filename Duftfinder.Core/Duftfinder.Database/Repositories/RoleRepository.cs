...
namespace Duftfinder.Database.Repositories
{
	...
	public class RoleRepository : Repository<Role, RoleFilter>, IRoleRepository
	{
		...
		public override FilterDefinition<Role> ApplyFilter(RoleFilter filter, IMongoCollection<Role> collection)
		{
			...
			var bsonFilter = Builders<Role>.Filter.Empty;
			var bsonFilterList = new List<FilterDefinition<Role>> {bsonFilter};
			...
			// Chain all filters.
			var queryFilter = Builders<Role>.Filter.And(bsonFilterList);
		...
		{
			var sortDefinitionBuilder = Builders<Role>.Sort;
			var sortDefinition = new List<SortDefinition<Role>>();
