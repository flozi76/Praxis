...
namespace Duftfinder.Database.Repositories
{
	...
	public class EffectMoleculeRepository : Repository<EffectMolecule, EffectMoleculeFilter>, IEffectMoleculeRepository
	{
		...
		/// <returns></returns>
		public override FilterDefinition<EffectMolecule> ApplyFilter(EffectMoleculeFilter filter,
			IMongoCollection<EffectMolecule> collection)
		{
			// Create list of filters and pass empty filter to list, should there be no custom filter.
			var bsonFilter = Builders<EffectMolecule>.Filter.Empty;
			var bsonFilterList = new List<FilterDefinition<EffectMolecule>> {bsonFilter};
			...
			if (!string.IsNullOrEmpty(filter.EffectId))
			{
				...
				bsonFilter =
					Builders<EffectMolecule>.Filter.Eq(nameof(EffectMolecule.EffectId), filter.EffectId.Trim());
			...
			if (!string.IsNullOrEmpty(filter.MoleculeId))
			{
				...
				bsonFilter =
					Builders<EffectMolecule>.Filter.Eq(nameof(EffectMolecule.MoleculeId), filter.MoleculeId.Trim());
				bsonFilterList.Add(bsonFilter);
			}

			// Chain all filters.
			var queryFilter = Builders<EffectMolecule>.Filter.And(bsonFilterList);
		...
		/// <param name="collection"></param>
		/// <returns></returns>
		public override SortDefinition<EffectMolecule> ApplySorting(EffectMoleculeFilter filter,
			IMongoCollection<EffectMolecule> collection)
		{
			var sortDefinitionBuilder = Builders<EffectMolecule>.Sort;
			var sortDefinition = new List<SortDefinition<EffectMolecule>>();
