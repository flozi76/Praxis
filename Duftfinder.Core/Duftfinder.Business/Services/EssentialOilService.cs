...
namespace Duftfinder.Business.Services
{
	...
	public class EssentialOilService : Service<EssentialOil, EssentialOilFilter, IEssentialOilRepository>,
		IEssentialOilService
	{
		...
		public EssentialOilService(IEssentialOilRepository essentialOilRepository,
			IEssentialOilEffectRepository essentialOilEffectRepository,
			IEssentialOilMoleculeRepository essentialOilMoleculeRepository, IEffectRepository effectRepository,
		...
		{
			var validationResult = await Repository.DeleteAsync(essentialOilId);
		...
		public async Task<IList<Molecule>> GetAssignedMoleculesForEssentialOilAsync(string essentialOilId)
		{
			...
			foreach (var essentialOilMolecule in essentialOilMolecules)
			{
				...
				var molecule = await _moleculeRepository.GetByIdAsync(essentialOilMolecule.MoleculeId);
		...
		public async Task<IList<Effect>> GetAssignedEffectsForEssentialOilAsync(string essentialOilId)
		{
			...
			var essentialOilEffects = await _essentialOilEffectRepository.GetByFilterAsync(new EssentialOilEffectFilter
				{EssentialOilId = essentialOilId});
			...
			foreach (var essentialOilEffect in essentialOilEffects)
			{
				...
				var effect = await _effectRepository.GetByIdAsync(essentialOilEffect.EffectId);
		...
		/// <returns></returns>
		public async Task<IList<SearchEssentialOilItem>> GetEssentialOilResultsBySearchedEffectsNameAsync(
			IList<SearchEffectItem> searchedEffects)
		{
			// List with all the essential oils of all searched effects. Is the result list.
			var searchEssentialOilItemsResults = new List<SearchEssentialOilItem>();

			// Do this for all searched effects.
			foreach (var searchedEffect in searchedEffects)
				...
				{
					var essentialOilEffects = await GetEssentialOilsForSearchedEffectAsync(searchedEffect);
					Log.Info(
					...
					if (essentialOilEffects.Count != 0)
					{
						...
						var searchEssentialOilItems = new List<SearchEssentialOilItem>();
						await CreateSearchEssentialOilItemsListAsync(searchEssentialOilItems, essentialOilEffects,
							searchedEffect);
		...
		private void JoinResult(List<SearchEssentialOilItem> searchEssentialOilItemsResults,
			List<SearchEssentialOilItem> searchEssentialOilItems)
		{
			// Add the search item to the result list, that contains all essential oils.
			foreach (var searchEssentialOilItem in searchEssentialOilItems)
			{
				// Check if search essential oil item already exists in result list.
				var searchEssentialOilItemInResultList =
					searchEssentialOilItemsResults.FirstOrDefault(s =>
						s.EssentialOilId == searchEssentialOilItem.EssentialOilId);
				...
				if (searchEssentialOilItemInResultList == null)
				{
					...
					searchEssentialOilItem.SearchEffectTextsInEssentialOil =
						searchEssentialOilItem.SearchEffectTextsInEssentialOil.Trim();
					searchEssentialOilItemsResults.Add(searchEssentialOilItem);

					Log.Info(
						$"Add new search essential oil item {searchEssentialOilItem.EssentialOil.Name} with id {searchEssentialOilItem.EssentialOilId} and with product of effect degree * discomfort value of {searchEssentialOilItem.EffectDegreeDiscomfortValue} to result list.");
				}
				else
				{
					// Recalculate EffectDegreeDiscomfortValue anew, if essential oil has already been added to result list.
					searchEssentialOilItemInResultList.EffectDegreeDiscomfortValue =
						searchEssentialOilItemInResultList.EffectDegreeDiscomfortValue +
					...
					searchEssentialOilItemInResultList.SearchEffectTextsInEssentialOil =
						$"{searchEssentialOilItemInResultList.SearchEffectTextsInEssentialOil};@{searchEssentialOilItem.SearchEffectTextsInEssentialOil.Trim()}";

					Log.Info(
						$"Essential oil item {searchEssentialOilItem.EssentialOil.Name} with id {searchEssentialOilItem.EssentialOilId} already exist in result list. Recalculate the the product of effect degree * discomfort value to {searchEssentialOilItemInResultList.EffectDegreeDiscomfortValue}.");
				}
			}
		}
			...
			SearchEffectItem searchedEffect)
		{
			...
			if (effects.Count == 0) return new List<EssentialOilEffect>();
